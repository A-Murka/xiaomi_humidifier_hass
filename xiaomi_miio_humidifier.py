...
SERVICE_SET_BUZZER_ON = 'xiaomi_miio_humidifier_set_buzzer_on'
SERVICE_SET_BUZZER_OFF = 'xiaomi_miio_humidifier_set_buzzer_off'
...
SERVICE_SET_LED_BRIGHTNESS = 'xiaomi_miio_humidifier_set_led_brightness'

AIRHUMIDIFIER_SERVICE_SCHEMA = vol.Schema({
    vol.Optional(ATTR_ENTITY_ID): cv.entity_ids,
})

SERVICE_SCHEMA_LED_BRIGHTNESS = AIRHUMIDIFIER_SERVICE_SCHEMA.extend({
    vol.Required(ATTR_BRIGHTNESS):
        vol.All(vol.Coerce(int), vol.Clamp(min=0, max=2))
})

#SERVICE_SCHEMA_FAVORITE_LEVEL = AIRHUMIDIFIER_SERVICE_SCHEMA.extend({
...
def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    ...
    def async_service_handler(service):
        ...
        if entity_ids:
            target_air_humidifiers = [air for air in hass.data[PLATFORM].values()
                                    if air.entity_id in entity_ids]
        else:
            target_air_humidifiers = hass.data[PLATFORM].values()

        update_tasks = []
        for air_humidifier in target_air_humidifiers:
            yield from getattr(air_humidifier, method['method'])(**params)
            update_tasks.append(air_humidifier.async_update_ha_state(True))
    ...
    descriptions = yield from hass.async_add_job(
        load_yaml_config_file, os.path.join(
            os.path.dirname(__file__), 'xiaomi_miio_humidifier_services.yaml'))
    ...
    for air_humidifier_service in SERVICE_TO_METHOD:
        ...
        hass.services.async_register(
            DOMAIN, air_humidifier_service, async_service_handler,
            description=descriptions.get(air_humidifier_service), schema=schema)
...
#                ATTR_MOTOR_SPEED: state.motor_speed
    ...
    def async_set_buzzer_on(self):
        """Turn the buzzer on."""
        yield from self._try_command(
            "Turning the buzzer of air humidifier on failed.",
    ...
    def async_set_buzzer_off(self):
        """Turn the buzzer on."""
        yield from self._try_command(
            "Turning the buzzer of air humidifier off failed.",
...
#        """Turn the led on."""
#        yield from self._try_command(
#            "Turning the led of air humidifier off failed.",
...
#        yield from self._try_command(
#            "Setting the favorite level of the air humidifier failed.",
#            self._air_humidifier.set_favorite_level, level)